...
func Test_patch_unkown_file(t *testing.T) {
	...
	dir := initTestGit()
	// When
	patch, err := services.GetPatchSinceCommit("", dir, "unkown.md")
	// Then
	i := is.New(t)
	i.Equal(patch, "")
	i.NoErr(err)
...
func Test_patch_line_changed(t *testing.T) {
	...
	// When
	patch, err := services.GetPatchSinceCommit("", dir, file)
	// Then
	i := is.New(t)
	i.NoErr(err)
...
func Test_patch_from_specific_commit(t *testing.T) {
	...
	// When
	patch, err := services.GetPatchSinceCommit(since, dir, file)
	// Then
	i := is.New(t)
	i.NoErr(err)
